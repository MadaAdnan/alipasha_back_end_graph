<div class="card card-body my-2 ">
    ...
    <form action="{{route('profile.store')}}" method="POST">
        ...
        <x-form.input-component :goto="route('plans.index')" helpText="يحتاج توثيق الحساب" name="face" :value="old('face')??auth()->user()->social['face']" label="رابط فيس بوك" :disabled="!auth()->user()->is_verified" placeholder="رابط فيس بوك" type="url"/>
        <x-form.input-component helpText="يحتاج توثيق الحساب" name="instagram" :value="old('instagram')??auth()->user()->social['instagram']" label="رابط إنستغرام" :disabled="!auth()->user()->is_verified" placeholder="رابط إنستغرام" type="url"/>
        <x-form.input-component helpText="يحتاج توثيق الحساب" name="tiktok" :value="old('tiktok')??auth()->user()->social['tiktok']" label="رابط تيك توك" :disabled="!auth()->user()->is_verified" placeholder="رابط تيك توك" type="url"/>
        <x-form.input-component helpText="يحتاج توثيق الحساب" name="twitter" :value="old('twitter')??auth()->user()->social['twitter']" label="رابط تويتر" :disabled="!auth()->user()->is_verified" placeholder="رابط تويتر" type="url"/>
        <x-form.input-component helpText="يحتاج توثيق الحساب" name="linkedin" :value="old('linkedin')??auth()->user()->social['linkedin']" label="رابط لينكد ان" :disabled="!auth()->user()->is_verified" placeholder="رابط لينكد ان" type="url"/>
