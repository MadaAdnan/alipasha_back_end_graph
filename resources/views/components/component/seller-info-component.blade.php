...
<div class="card card-body seller-data">
    <div class="d-flex justify-content-between align-items-center">
        <h6 class="text-gray fw-bold my-1">معلومات المعلن @if($seller->id==auth()->id())
                <a href="{{route('profile.index')}}"><i class="fa fa-edit"></i></a>
            @endif</h6>
        @auth
            @php
            ...
            @endphp
            <form @if(!$isFollowing) action="{{route('following-to-seller',$seller->id)}}"
                  @else action="{{route('unfollowing-to-seller',$seller->id)}}" @endif method="POST">
                @csrf
    ...
    <div class="d-flex justify-content-center gap-1">
        @auth
            ...
            @else
                <button class="btn-green rounded bg-transparent d-flex justify-content-center align-items-center"
                        onclick="showToast('لا يمكنك إجراء محادثة مع نفسك','error')">
                    <i class="fa fa-comments text-black"></i>
                    <span class="small d-none d-md-inline-block  text-black">
                   تحدث معه
                </span>
        ...
        @else
            <button class="btn-green rounded bg-transparent d-flex justify-content-center align-items-center"
                    onclick="showToast('يرجى تسجيل الدخول اولاً','error')">
                <i class="fa fa-comments text-black"></i>
                <span class="small d-none d-md-inline-block  text-black">
                   تحدث معه
                </span>
        ...
        @else
            <a class="btn-green rounded d-flex justify-content-center align-items-center"
               href="https://wa.me/{{$seller->full_phone}}" target="_blank">
                <i class="fa-brands fa-whatsapp"></i>
                <span class="small d-none d-md-inline-block  text-white">
                   واتس آب
                </span>
